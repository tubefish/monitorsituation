...
test('switches lazily between situation, Wingbits, and location news', () => {
  ...
  assert.equal(map.querySelector('iframe'), null);
  const flights = section.querySelector<HTMLButtonElement>('[data-map-experience="flights"]')!;
  flights.click();
  const frame = map.querySelector<HTMLIFrameElement>('iframe')!;
  ...
  assert.equal(map.querySelector<HTMLElement>('.map-experience-news')!.hidden, false);
  map.querySelector<HTMLButtonElement>('.map-experience-news-focus')!.click();
  assert.equal(selected, 'Top story');
  assert.equal(map.querySelectorAll('iframe').length, 1, 'Wingbits stays mounted when switching views');
